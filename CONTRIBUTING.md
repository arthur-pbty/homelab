...
- Ne jamais committer de secrets (voir `terraform/terraform.tfvars.example` et `ansible/inventory.ini.example`).
- Utiliser `ansible-vault` pour chiffrer les secrets Ansible.
- Pour Terraform, configurez un backend distant (S3/GCS) pour l'état si le projet s'agrandit.
